...
class BaseDataset(Dataset):
    def __init__(self, 
        ...
        for i, entry in enumerate(self.anno):
            ...
            bev_mf_paths = self.scene_anno[i]['frames']
...
def create_dataset(config):
    ...
    for k, v in val_files.items():
        ...
        for val_file in v:
            ...
            print(f"\n--- First 10 samples from dataset '{k}' ---")
            for i in range(min(10, len(dataset))):
                sample = dataset[i]
                print(f"Sample {i}:")
                print(f"  Prompt        : {sample[0]}")
                print(f"  Ref Captions  : {sample[1]}")
                print(f"  Scene ID      : {sample[2]}")
                print(f"  BEV MF Paths  : {sample[3]}")
                print(f"  QID           : {sample[4]}")
                print(f"  Obj ID        : {sample[5]}")
                print(f"  Pred ID       : {sample[6]}")
                print(f"  Type Info     : {sample[7]}\n")
            #assert False, 'debug'
